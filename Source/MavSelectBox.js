...
var MavSelectBox = new Class({
	...
	initialize: function(_options) {
		...
		this.element = document.id(this.options.elem);
		this.elementCopy = this.element.clone().set({
			'id':this.element.get('id'),
			'name':this.element.get('name')
		});

		if (!$defined(this.options.container) || (this.container = document.id(this.options.container)) == null) {
			this.container = null;
		}
	...
	destroy: function(_revert) {
		if (_revert) {
			...
			this.elementCopy.replaces(document.id(this.element.get('id')));
		}
	...
	create_option: function(_opt, _idx, _group) {
		...
		}).inject((document.id(_group) || this.elementOptions));
	...
	determine: function(_elem) {
		...
		if ($type(_elem) != 'element') {
			...
			elem = elem.filter(function(_el) {
						return (!document.id(_el).hasClass(this.options.disableClass) && 
								((_elem == 'up' && document.id(_el).retrieve('idx') < fromIdx ) ||
								 (_elem == 'down' && document.id(_el).retrieve('idx') > fromIdx)));
			}, this);
	...
	over: function(e) {
		e = new Event(e);
		var elem = (document.id(e.target).get('tag') != 'li' ? document.id(e.target).getParent('li') : document.id(e.target));
